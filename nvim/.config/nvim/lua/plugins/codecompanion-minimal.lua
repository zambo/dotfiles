...
-- I use 1Password's new environments feature to set my FUEL_IX_API_KEY env var
...
local function get_fuel_ix_models()
  ...
  log_debug(string.format("API request failed with status %s, using fallback models", res.status or "unknown"))
...
return {
  {
    ...
    opts = {
      ...
      -- ──────────────────────────────────────────────────────────────────
      extensions = {
        -- Leaving this clean to keep the example minimal, but I use only History and MCP
