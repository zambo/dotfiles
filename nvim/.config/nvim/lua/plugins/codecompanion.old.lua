...
--
-- TODO:s (things I want to improve):
-- - [ ] Implement configurable cache expiration
-- - [ ] Add comprehensive error handling and logging
-- - [ ] Make caching optional via config
-- - [ ] Add model metadata caching (pricing, context limits, capabilities)
-- - [ ] Implement retry logic for API failures
-- - [x] Add health check command to validate API key and connectivity
...
local DEBUG = os.getenv("FUELIX_DEBUG") == "1"
...
local FUELIX_API_KEY = os.getenv("FUEL_IX_API_KEY") or "your_api_key_here"
...
local function refetch_fuel_ix_models()
  ...
  vim.notify("Fuel iX model cache cleared. Models will be refetched on next use.", vim.log.levels.INFO)
end
...
return {
  {
    ...
    opts = {
      ...
      interactions = {
        chat = {
          -- I prefer Sonnet for chat - it's more thoughtful and detailed
          adapter = { name = "fuel_ix", model = MODELS.SONNET },
          variables = {
            ["buffer"] = {
              opts = {
                -- Only share diffs by default to save on tokens
                -- Use "all" if you need the full buffer context
                default_params = "diff",
              },
            },
          },
          ...
          opencode = function()
            return require("codecompanion.adapters").extend("opencode", {
              commands = {
                ...
                fuel_ix_gpt_4_1 = { "opencode", "acp", "-m", "fuel_ix/gpt-4-1" },
                fuel_ix_gpt_5_2 = { "opencode", "acp", "-m", "fuel_ix/gpt-5-2" },
              },
            })
          end,
        },
        ...
        http = {
          fuel_ix = function()
            ...
            return {
              ...
              opts = {
                -- Streaming disabled for now - causes annoying markdown rendering issues
                -- Will re-enable once that's fixed
                stream = false,
      ...
      prompt_library = {
        markdown = {
          dirs = {
            -- I keep my custom prompts in my nvim config
            "~/.config/nvim/prompts",
          },
        },
      },
      ...
      extensions = {
        history = {
          ...
          opts = {
            ...
            auto_save = true,
            expiration_days = 0, -- Never expire - I like keeping all my chats
            picker = "snacks", -- Using snacks.nvim for the picker UI
            auto_generate_title = false, -- Disabled - I prefer manual titles
